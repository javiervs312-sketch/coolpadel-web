...
def build_centered_header_site():
    html = """<!DOCTYPE html>
    ...
    tailwind.config = {
      ...
      theme: {
        extend: {
          ...
          animation: {
            'ticker-slow': 'tickerSlow 32s linear infinite',
...
<body class="bg-[#0b1626] text-slate-100 min-h-screen relative selection:bg-cool-orange selection:text-white overflow-x-hidden">
  ...
  <!-- 1. TOP BAR NEGRA: SOLO EL INFORME EN BUCLE ESPACIADO -->
  <div class="bg-black border-b border-neutral-800 py-2.5 ticker-wrap text-xs text-neutral-200 tracking-widest z-50 relative">
    <div class="ticker-content animate-ticker-slow flex items-center font-medium">
      ...
      <!-- DUPLICADO EXACTO PARA BUCLE CONTINUO SUAVE -->
      <a href="#informe" class="inline-flex items-center hover:text-white transition px-10">
        <span class="w-1.5 h-1.5 rounded-full bg-cool-orange mr-3"></span>
        <span>Descarga el informe exclusivo sobre la industria del padel</span>
      </a>
  ...
  <!-- 2. NAVBAR BLANCA: LOGO EN EL CENTRO + BOTÓN NARANJA/LETRA NEGRA A LA DERECHA -->
  <header class="sticky top-0 z-40 bg-white text-slate-900 border-b border-slate-200 shadow-md">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
      
      <!-- ESPACIO IZQUIERDO PARA CENTRADO EXACTO -->
      <div class="w-24 sm:w-32 hidden sm:block"></div>

      <!-- LOGO: MASCOTA + LETRAS EN EL MEDIO -->
      <a href="#" class="flex items-center justify-center gap-2.5 sm:gap-3.5 group py-2 mx-auto sm:mx-0">
        <img src="assets/images/coolpadel-mascot-hd.png" alt="Mascota CoolPadel" class="h-11 sm:h-13 w-auto object-contain group-hover:scale-105 transition max-h-[52px]">
        <img src="assets/images/coolpadel-typography-hd.png" alt="CoolPadel" class="h-6 sm:h-7 w-auto object-contain">
      </a>
      ...
      <div class="flex items-center justify-end w-auto sm:w-32">
        <a href="https://wa.me/34680317486?text=Hola%20Javier" 
           target="_blank" 
           class="px-5 sm:px-6 py-2.5 rounded-full font-heading font-black text-xs sm:text-sm bg-cool-orange text-slate-950 hover:bg-cool-amber active:scale-95 shadow-md hover:shadow-lg transition flex items-center gap-2">
          <span>Contactar</span>
  ...
  <section class="relative bg-black overflow-hidden select-none">
    ...
    <div id="slider-container" class="relative w-full overflow-hidden">
      ...
      <div id="slider-track" class="slider-track">
        ...
        <!-- SLIDE 1: LLAVEROS PARA CLUBS (HD PHOTO) -->
        <div class="slide-item relative h-[600px] sm:h-[700px] lg:h-[760px] cursor-pointer" onclick="document.querySelector('#llaveros').scrollIntoView({behavior: 'smooth'})">
          <img src="assets/images/coolpadel-hd-banner.jpg" alt="Llaveros de pádel CoolPadel" class="w-full h-full object-cover">
  ...
  <script>
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        f.write(html)
    print("Updated index.html with centered mascot/logo and orange contact button.")

if __name__ == "__main__":
    build_centered_header_site()
