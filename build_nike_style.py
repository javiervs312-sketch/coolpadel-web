    ...
    tailwind.config = {
      ...
      theme: {
        extend: {
          ...
          animation: {
            'ticker-slow': 'tickerSlow 35s linear infinite',
...
<body class="bg-[#0b1626] text-slate-100 min-h-screen relative selection:bg-cool-orange selection:text-white overflow-x-hidden">
  ...
  <!-- 1. TOP BAR NEGRA ESTILO SECCO / TICKER ELEGANTE Y ESPACIADO DEL INFORME -->
  <div class="bg-black border-b border-neutral-800 py-2.5 ticker-wrap text-xs text-neutral-200 tracking-wide z-50 relative">
    <div class="ticker-content animate-ticker-slow flex items-center gap-16 font-medium">
      
      <a href="#informe" class="inline-flex items-center gap-3 hover:text-white transition">
        <span class="w-2 h-2 rounded-full bg-cool-orange"></span>
        <span>Informe del Negocio del Pádel gratis</span>
        <span class="text-cool-cyan underline font-bold">Descargar PDF</span>
      </a>
      <span class="text-neutral-600">|</span>

      <a href="#informe" class="inline-flex items-center gap-3 hover:text-white transition">
        <i data-lucide="book-open" class="w-3.5 h-3.5 text-yellow-400"></i>
      ...
      <a href="#informe" class="inline-flex items-center gap-3 hover:text-white transition">
        ...
        <span>Análisis PWS Barcelona & RacquetX Miami</span>
      </a>
      <span class="text-neutral-600">|</span>

      <!-- DUPLICADO PARA BUCLE INFINITO CONTINUO -->
      <a href="#informe" class="inline-flex items-center gap-3 hover:text-white transition">
        <span class="w-2 h-2 rounded-full bg-cool-orange"></span>
        <span>Informe del Negocio del Pádel gratis</span>
        <span class="text-cool-cyan underline font-bold">Descargar PDF</span>
  ...
  <!-- 2. NAVBAR BLANCA ESTILO NIKE CON TEXTO EN LAS POSICIONES ACTUALES -->
  <header class="sticky top-0 z-40 bg-white text-slate-900 border-b border-slate-200 shadow-md">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
      
      <!-- LOGO COOLPADEL -->
      <a href="#" class="flex items-center gap-3 group">
        <div class="relative w-11 h-11 bg-gradient-to-tr from-cool-blue to-cool-cyan rounded-full flex items-center justify-center p-1 shadow-md group-hover:scale-105 transition">
          <svg viewBox="0 0 100 100" class="w-full h-full">
            ...
            <circle cx="38" cy="48" r="6" fill="#091322"/>
            <circle cx="62" cy="48" r="6" fill="#091322"/>
            <circle cx="40" cy="46" r="2" fill="#ffffff"/>
            <circle cx="64" cy="46" r="2" fill="#ffffff"/>
            <path d="M 44 62 Q 50 68 56 62" stroke="#091322" stroke-width="3.5" fill="none" stroke-linecap="round"/>
          </svg>
        </div>
        <div>
          <span class="font-heading font-black text-2xl tracking-tight text-slate-950 flex items-center">
            COOL<span class="text-cool-orange">PADEL</span>
          </span>
          <span class="text-[10px] tracking-wider uppercase text-slate-500 font-extrabold block -mt-1">Padel Hub & Club Solutions</span>
        </div>
      </a>

      <!-- NAV LINKS (CENTRADOS / POSICIONES ACTUALES) -->
  ...
  <!-- 3. HERO SLIDER ESTILO NIKE (3 PANTALLAS QUE SE DESPLAZAN A LA DERECHA) -->
    ...
    <div id="slider-container" class="relative w-full overflow-hidden">
      ...
      <div id="slider-track" class="slider-track">
        ...
        <div class="slide-item relative h-[600px] sm:h-[700px] lg:h-[760px] cursor-pointer" onclick="document.querySelector('#llaveros').scrollIntoView({behavior: 'smooth'})">
          ...
          <!-- Dark overlay gradient at bottom & left for Nike style text contrast -->
  ...
  <footer class="bg-black border-t border-neutral-800 py-10 text-center text-xs text-neutral-400">
    <div class="max-w-7xl mx-auto px-4 space-y-2">
      ...
      <p class="text-neutral-500">Diseñado con pasión por el pádel.</p>
    </div>
  </footer>
  ...
  <script>
    ...
    with open(out_file, "w", encoding="utf-8") as f:
        f.write(html)
    print("Nike style CoolPadel site generated successfully in coolpadel-web/index.html")

if __name__ == "__main__":
    build_nike_style_site()
